...
x_axis = [1, 2, 3, 4, 5, 8, 10, 15, 20]
logfilename = "logfile_omega_plot" + str(grid_size) + "_omega_" + str(omega) + "_tol_" + str(tol)
logfile = open(logfilename, "w", 0)

filenames_hapod = ["/scratch/tmp/l_tobi01/pickled_bases/hapod_" + hapod_type + "_" + str(grid_size)  for hapod_type in hapod_types]
filename_pod = "/scratch/tmp/l_tobi01/pickled_bases/pod_gridsize_" + str(grid_size)

for chunk_size in x_axis:
    b = HapodBasics(grid_size, chunk_size, epsilon_ast=tol, omega=omega)
    if b.rank_world == 0:
        print("Current omega:", omega)
        logfile.write("Current omega:" + str(omega) + "\n")
    for num, hapod_type in enumerate(hapod_types):
        b = HapodBasics(grid_size, chunk_size, epsilon_ast=tol, omega=omega)
    
        try:
            f = open(filenames_hapod[num] + "_chunk_" + str(chunk_size) + "_omega_" + str(omega) + "_tol_" + str(tol), "rb")
            if b.rank_world == 0:
            ...
            if b.rank_world == 0:
                f = open(filenames_hapod[num] + "_chunk_" + str(chunk_size) + "_omega_" + str(omega) +  "_tol_" + str(tol), "wb")
...
if b.rank_world == 0:
    ...
    filename = "chunk_num_modes_gridsize_%d_chunk_%d_tol_%g" % (b.gridsize, chunk_size, tol)
    ...
    filename = "chunk_l2_mean_errs_gridsize_%d_chunk_%d_tol_%g" % (b.gridsize, chunk_size, tol)
    ...
    filename = "chunk_time_gridsize_%d_chunk_%d_tol_%g" % (b.gridsize, chunk_size, tol)
    ...
    plt.savefig("omega_memory_gridsize_" + str(b.gridsize) + "_chunk_" + str(chunk_size) + "_tol__" + str(tol) + ".png")
    plt.clf()
