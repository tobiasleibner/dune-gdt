...
def boltzmann_all_binary_tree_hapod(grid_size, chunk_size, tol, omega=0.95, logfile=None, incremental_gramian=True):
    ...
    core_binary_tree_depth = binary_tree_depth(mpi.comm_proc)
    ...
    rooted_tree_depth = core_binary_tree_depth + num_chunks-1 + node_binary_tree_depth-1
    ...
    for i in range(num_chunks):
        ...
        timestep_vectors, timestep_svals, num_snaps_in_this_chunk, max_vectors_before_pod_in_hapod, max_local_modes_in_hapod \
        ...
        if mpi.rank_proc == 0:
            total_num_snapshots += num_snaps_in_this_chunk
            if i == 0:
                modes, svals = [timestep_vectors, timestep_svals]
            else:
                max_vectors_before_pod = max(max_vectors_before_pod, len(modes) + len(timestep_vectors))
                modes, svals = local_pod([[modes, svals], [timestep_vectors, timestep_svals]], total_num_snapshots,
...
if __name__ == "__main__":
    ...
    filename = "HAPOD_all_binary_tree_gridsize_%d_chunksize_%d_tol_%f_omega_%f" % (grid_size, chunk_size, tol, omega)
    logfile = open(filename, "a")
    final_modes, _, total_num_snapshots, mu, mpi, _, _, _ = boltzmann_all_binary_tree_hapod(grid_size, chunk_size,
                                                                                        tol * grid_size,
