...
filenames_hapod = ["/scratch/tmp/l_tobi01/pickled_bases/hapod_" + hapod_type + "_" + str(grid_size) + "_chunk_" + str(chunk_size)  for hapod_type in hapod_types]
filename_pod = "/scratch/tmp/l_tobi01/pickled_bases/pod_gridsize_" + str(grid_size)
x_axis = [0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99, 0.999]
logfilename = "logfile_omega_plot" + str(grid_size) + "_chunk_" + str(chunk_size) + "_tol_" + str(tol)
...
for omega in x_axis:
    ...
    for num, hapod_type in enumerate(hapod_types):
        ...
        try:
            f = open(filenames_hapod[num] + "_omega_" + str(omega) + "_tol_" + str(tol), "rb")
            if b.rank_world == 0:
            ...
            if b.rank_world == 0:
                f = open(filenames_hapod[num] + "_omega_" + str(omega) +  "_tol_" + str(tol), "wb")
...
if b.rank_world == 0:
    ...
    filename = "omega_num_modes_gridsize_%d_chunk_%d_tol_%g" % (b.gridsize, chunk_size, tol)
    ...
    filename = "omega_l2_mean_errs_gridsize_%d_chunk_%d_tol_%g" % (b.gridsize, chunk_size, tol)
    ...
    filename = "omega_time_gridsize_%d_chunk_%d_tol_%g" % (b.gridsize, chunk_size, tol)
